...
local function updateModelStats(wgt)
    ...
    local num_flights
    if (wgt.options.flightsCounterSource == 1) then -- use flight widget counter
        num_flights = flight_count_from_flight_widget
    else -- use RF2 firmware counter or fallback to flight widget counter if RF2 firmware counter is not available
